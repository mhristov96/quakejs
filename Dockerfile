...
# Update apt-get and install essential tools like curl, gpg, git, nginx, supervisor, and Certbot
RUN apt-get update && \
    apt-get install -y curl gpg git nginx supervisor certbot python3-certbot-nginx
...
COPY fullchain.pem /etc/letsencrypt/live/quakejs.mirolife.world/fullchain.pem
COPY privkey.pem /etc/letsencrypt/live/quakejs.mirolife.world/privkey.pem  
